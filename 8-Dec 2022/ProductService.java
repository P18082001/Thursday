	...
	if(f==true)
	{
		...
		if(product[i].getId()==id)
		{
			...
			if(product[i].getQuantity()>quantity) //10>1
			{
				...
				//updating stock after booking product-after oredr
			    product[i].setQuantity(product[i].getQuantity()-quantity);
				order[orderIndex]=new OrderDetails(name, product[i].getBrand(), quantity, total);
			    orderIndex++;
			    break;
			}
		}
...
//this method is responsible for delete product by Product id
static void deleteProductById()
{
	boolean status=false;
	System.out.println("Enter id to delete Product details");
	int id=sc.nextInt();
	for(int i=0;i<index;i++)
	{
		if(id==product[i].getId())
		{
			...
			index--;
			System.out.println("Product Item delete Successfully!!!");
			status=true;
			break;
		}
	}
...
static void cancelOrder()
{
	...
	for(int i=0;i<orderIndex;i++)
	{
		if(OId==product[i].getId())//1==order[0]1
		{
